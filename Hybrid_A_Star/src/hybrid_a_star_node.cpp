...
void HybridAstarNode::Run() {
    ...
    while (HasStartPose() && HasGoalPose()) {
        ...
        if (kinodynamic_astar_searcher_ptr_->Search(start_state, goal_state, use_dubins_or_rs_)) {
            ...
            // Save path to file
            std::ofstream path_file("scripts/path.txt");
            if (path_file.is_open()) {
                for (const auto& state : path) {
                    path_file << state.x() << " " << state.y() << " " << state.z() << std::endl;
                }
                path_file.close();
            }
            ...
            std::ofstream quintic_path_file("scripts/quintic_path.txt");
            if (quintic_path_file.is_open()) {
                for (const auto& state : quintic_path) {
                    quintic_path_file << state.x() << " " << state.y() << " " << state.z() << std::endl;
                }
