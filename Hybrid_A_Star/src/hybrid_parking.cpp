...
HybridParking::HybridParking(ros::NodeHandle &nh) {
    ...
    obstacle_points_sub_ = nh.subscribe("/obstacle_points", 10, &HybridParking::ObstaclePointsCallback, this);
    has_map_ = false;
    has_sdf_map_ = false;
}

void HybridParking::ObstaclePointsCallback(const sensor_msgs::PointCloud2ConstPtr& point_cloud_msg) {
    obstacle_points_deque_.push_back(point_cloud_msg);
}

void HybridParking::Run() {
    ReadData();
    ...
    while (HasStartPose() && HasGoalPose()) {
        ...
        // Vec3d goal_state = Vec3d(11.8, 8.65, 3.1416);
        ...
        if (kinodynamic_astar_searcher_ptr_->Search(start_state, goal_state, use_dubins_or_rs_)) {
            ...
            PublishVehiclePath(path, 2.0, 1.0, 5u);
            // PublishVehiclePath(path, 2.0, 1.0, 3u);
            VehicleMotionPath(path, 0.03);
        }
        else{
            // 打印红色提示
...
void HybridParking::VehicleMotionPath(const VectorVec3d &path, double sleep_duration) {
...
VectorVec3d HybridParking::RefinePath(const VectorVec3d& smooth_path, int original_size) {
...
void HybridParking::InitPoseData() {
...
bool HybridParking::HasGoalPose() {
    return !goal_pose_deque_.empty();
}

bool HybridParking::HasStartPose() {
    return !init_pose_deque_.empty();
}

void HybridParking::PublishPath(const VectorVec3d &path) {
...
void HybridParking::PublishSmoothPath(const VectorVec3d &path) {
    nav_msgs::Path nav_path;
...
void HybridParking::PublishVehiclePath(const VectorVec3d &path, double width,
...
void HybridParking::PublishSearchedTree(const VectorVec4d &searched_tree) {
