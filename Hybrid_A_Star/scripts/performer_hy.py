...
def calculate_derivatives_and_integral(data, dt):
    ...
    calcs = {'integrated_acc': integrated_acc_sq, 'k': k, 'd': dist_obs, 'a_mag': a_mag}
    times = {'t': t}
...
col_ours = '#2ca02c'  # 绿色
col_base = '#d62728'  # 红色
limit_style = {'color': 'black', 'linestyle': '--', 'linewidth': 1.5, 'alpha': 0.8}
...
# ax.set_title('(a) Integrated Squared Acceleration', fontsize=FRONTSIZE, pad=10)
# ax.set_ylabel(r'$J(t) = \int_{0}^{t} |a(\tau)|^2 d\tau$', fontsize=FRONTSIZE)
# ax.set_xlabel(r'$t \, (s)$', fontsize=FRONTSIZE)
# ax.set_xlim(0, t_max)
# ax.grid(True, linestyle=':', alpha=0.6)
# ax.legend(loc='upper left', frameon=True, fontsize=FRONTSIZE)


# --- 子图 1: 加速度幅值 ---
ax = axs[0]
if 'a_mag' in calcs_base:
    ax.plot(times_base['t'], calcs_base['a_mag'], color=col_base, label='Initial Path', linewidth=line_width, alpha=0.8)
if 'a_mag' in calcs_ours:
    ax.plot(times_ours['t'], calcs_ours['a_mag'], color=col_ours, label='Optimized Trajectory', linewidth=line_width)
# # 可选：添加加速度限制线
# a_limit = 2.0  # 根据实际需求设置加速度限制 (m/s^2)
# ax.hlines(a_limit, 0, t_max, **limit_style)
ax.set_title('(a) Acceleration Magnitude', fontsize=FRONTSIZE, pad=10)
ax.set_xlabel(r'$t \, (s)$', fontsize=FRONTSIZE)
ax.set_ylabel(r'$|a(t)| \, (m/s^2)$', fontsize=FRONTSIZE)
ax.set_xlim(0, t_max)
# 动态调整Y轴范围
a_max_val = 0
if 'a_mag' in calcs_base and calcs_base['a_mag'].size > 0:
    a_max_val = max(a_max_val, np.max(calcs_base['a_mag']))
if 'a_mag' in calcs_ours and calcs_ours['a_mag'].size > 0:
    a_max_val = max(a_max_val, np.max(calcs_ours['a_mag']))
ax.grid(True, linestyle=':', alpha=0.6)
ax.legend(loc='upper left', frameon=True, fontsize=FRONTSIZE)
ax.tick_params(axis='both', which='major', labelsize=FRONTSIZE)
# ax.text(t_max * 0.6, a_limit + 0.1, r'$a_{max}$', fontsize=FRONTSIZE)
...
ax.set_title('(b) Curvature Profile', fontsize=FRONTSIZE, pad=10)
ax.set_ylabel(r'$\kappa \, (m^{-1})$', fontsize=FRONTSIZE)
ax.set_xlabel(r'$t \, (s)$', fontsize=FRONTSIZE)
ax.set_xlim(0, t_max)
ax.set_ylim(-0.6, 0.6)
ax.grid(True, linestyle=':', alpha=0.8)
ax.text(t_max * 0.6, 0.32, r'$\kappa_{max} = 0.3$', fontsize=FRONTSIZE)
ax.tick_params(axis='both', which='major', labelsize=FRONTSIZE)
...
ax.set_title('(c) Distance to Obstacle', fontsize=FRONTSIZE, pad=10)
ax.set_xlabel(r'$t \, (s)$', fontsize=FRONTSIZE)
ax.set_ylabel(r'$d_{obs} \, (m)$', fontsize=FRONTSIZE)
ax.set_xlim(0, t_max)
# 动态调整Y轴范围
d_max_val = 0
...
ax.grid(True, linestyle=':', alpha=0.8)
ax.text(t_max * 0.1, 0.5, r'$d_{safe} = 0.8$', fontsize=FRONTSIZE)
ax.tick_params(axis='both', which='major', labelsize=FRONTSIZE)
